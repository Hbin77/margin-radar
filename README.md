...
- **AI 가격예측·이상탐지**: 조달 45만건 기반 앙상블(HistGBM+RandomForest→Ridge, R²=0.948)로
  적정가 예측 + 과지출 탐지(out-of-fold 잔차) + KMeans 시장세분화 + IsolationForest 이상거래
...
## 현재 규모 (매일 갱신)
- 수집 45.5만 행 / 829 품명 → **258 품목** 랭킹
- ML: 앙상블 R²=0.948 / HGB 0.941, MAPE 21.0% (학습 표본 15만 상한 / 수집 45.5만)
- 검증: 룩업 베이스라인(R²=0.840) 대비 MAE +33%, GroupKFold(미지카테고리) R²=0.41
